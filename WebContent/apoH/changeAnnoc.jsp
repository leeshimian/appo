		...
		<div id="bodyy">
			...
			<div id="right">
				...
				<%
					...
					connDB.getConn();
					String sql = "select*from annoc where  annoc_id=?";
					ResultSet rs = connDB.connQuery( sql,typeid);
					rs.next();
				%>
				
				<div id="c2">
					<form action="do_changeAnnoc.jsp?id=<%=rs.getString("annoc_id") %>" method="post">
						<table cellspacing="0px" width="500px" height="200px" border="1px" style=" margin-top: 60px; border-color:cornflowerblue;">
							<tr>
								<td>公告标题</td>
								<td><input type="text" name="newtitle" value="<%=rs.getString("annoc_title") %>" /></td>
							</tr>
							<tr>
								<td>公告内容</td>
								<td>
								<textarea id="sname" name="newtext" required="required" cols="20" rows="10" wrap="hard">
									<%=rs.getString("annoc_text") %>
								</textarea>
								</td>
							</tr>
							<tr>
